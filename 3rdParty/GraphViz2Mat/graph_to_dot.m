...
%
%
%slightly extended by urut/may2012
%added more layout options urut/jan2015
%
%

node_label = [];   
arc_label = [];   % set default args
...
for i = 1:2:nargin-1              % get optional args
    switch varargin{i}
        ...
        case 'node_color', node_color = varargin{i+1};
        ...
        case 'overalp', overlap = varargin{i+1};
        case 'splines', splines = varargin{i+1};
        case 'nodesep', nodesep = varargin{i+1};
...
fprintf(fid, 'splines=%s\n', splines );
fprintf(fid, 'nodesep=%s\n', nodesep);
...
for node = 1:Nnds               % process NODEs 
    if isempty(node_label)
        ...
        if isempty(node_color)
            fprintf(fid, '%d [ label = "%s" ];\n', node, node_label{node});
        else
            fprintf(fid, '%d [ label = "%s",fillcolor=%s,style=filled];\n', node, node_label{node}, node_color{node});
